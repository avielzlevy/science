...
> = {
  ...
  physics: [
    { start: 0, end: 3, text: "Quantum entanglement defies logic." },
    {
      ...
      text: "Change the spin of a particle here, and its twin flips across the galaxy.",
    },
    {
      start: 6,
      end: 10,
      text: "Einstein called it 'spooky action'. He was right to be afraid.",
    },
  ],
  biology: [
    ...
    { start: 3, end: 6, text: "we found an organism that doesn't use DNA." },
    {
      ...
      text: "It uses pure silicon. It's essentially a living machine.",
    },
  ],
  computers: [
  ...
  chemistry: [
    { start: 0, end: 3, text: "Drop francium into water." },
    {
      ...
      text: "The reaction is so violent it shatters the glass.",
    },
...
export function Stage({
  ...
  const safeCategory =
    category && TRANSCRIPTS[category] ? category : "wildcard";
  const activeTranscript = skit
    ? [
        {
          start: 0,
          end: 5,
          text: skit.worldBefore || "The world as we knew it…",
        },
        {
          start: 5,
          end: 10,
          text: skit.breakthrough || "Then everything changed…",
        },
        {
          start: 10,
          end: 15,
          text: skit.newReality || "A new reality emerged…",
        },
      ]
    ...
    >
      ...
      <div className="absolute top-8 left-8 z-50 flex items-center gap-3 px-5 py-3 rounded-full border border-black/10 bg-white/80 backdrop-blur-3xl shadow-sm text-black">
        {reliabilityScore > 80 ? (
          ...
          <ShieldAlert
            className="text-orange-500 animate-[pulse_2s_ease-in-out_infinite]"
            size={20}
          />
        )}
      ...
      {loading && (
        <div className="absolute inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-xl">
          <div className="flex flex-col items-center gap-6">
            ...
            <p className="text-white/70 font-body text-lg tracking-wide">
      ...
      {error && !loading && (
        <div className="absolute inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-xl">
          <div className="flex flex-col items-center gap-6 max-w-md text-center">
            <p className="text-white/90 font-heading font-bold text-2xl">
              Pipeline Warming Up
            </p>
            <p className="text-white/60 font-body text-lg">{error}</p>
            <button
              onClick={onClose}
              className="mt-4 px-8 py-3 rounded-full bg-white text-black font-bold uppercase tracking-widest hover:scale-105 transition-all"
            >
              Go Back
            </button>
          </div>
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-4xl px-8">
        <div className="perspective-1000 mb-16">
          ...
          >
            ...
            <div
              className="absolute top-12 left-12 w-24 h-1 bg-white/20 rounded-full"
              style={{ transform: "translateZ(20px)" }}
            />
            <div
              className="absolute bottom-12 right-12 w-16 h-1 bg-white/20 rounded-full"
              style={{ transform: "translateZ(20px)" }}
            />

            {skit?.coverArtUrl && (
              <img
                src={skit.coverArtUrl}
                alt="Cover art"
            ...
            >
              {skit?.research?.title ||
                category ||
                (isWildcard ? "Wildcard Data" : "Research File")}
            </motion.div>
          </motion.div>
        </div>
          ...
          {activeTranscript.map((line, idx) => {
            ...
            return (
              <motion.p
                ...
                style={{
                  visibility:
                    isActive || isPast || idx === 0 ? "visible" : "hidden",
                }}
              >
                {line.text}
              </motion.p>
